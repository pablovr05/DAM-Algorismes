...
def classify_texts(texts, model, vectorizer, device):
    # Convert texts to vectorized input
...
def main():
    ...
    label_encoder = {k: v for v, k in label_decoder.items()}
    ...
    for i, text in enumerate(texts):
        ...
        probs_str = " ,".join([f"{class_names[j][:5]}:{prob:5.2f}" for j, prob in enumerate(probabilities[i])])
        print(f"{probs_str}")
