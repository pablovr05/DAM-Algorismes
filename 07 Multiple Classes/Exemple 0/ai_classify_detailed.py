...
with torch.no_grad():
    for img_path, label in test_images:
        ...
        probabilities = F.softmax(outputs, dim=1)[0]  # Aplica softmax per obtenir les probabilitats
        ...
        # Mostra les probabilitats de cada classe
        print(f"Image: {img_path}, Predicted: {"'"+predicted_class+"'":8} ({"'"+label+"'":8} : {match})")
        for i, prob in enumerate(probabilities):
            print(f"  {class_names[i]:8}: {prob.item() * 100:.2f}%")
